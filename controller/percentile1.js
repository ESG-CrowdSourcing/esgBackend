...
async function companyDetails(dp, y, companyName) {
    return new Promise(async (resolve, reject) => {
        ...
        for (let i = 0; i < companyName.length; i++) {
            let dpValu = await clientData.find({ DPCode: dp, fiscalYear: y, companyName: companyName[i] }).distinct('response').exec()
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let i = 0; i < year.length; i++) {
                for (let j = 0; j < dpCodes.length; j++){
                    let dpValues = await companyDetails(dpCodes[j], year[i], companyName);
                    for (let k = 0; k < companyName.length; k++){
                        console.log('/???????????????????????????????????' , dpCodes.length , dpValues , year[i] );
                        let polarityCheck = await data.find({ DPCode: dpCodes[j], percentile: 'Yes' }).distinct('polarity').exec()
                        console.log(";;;;;;;;;;;;;;;;;;;;;;;;;;; , ................. llllllllllllllllllll " , polarityCheck)
                        let response = await clientData.find({ DPCode: dpCodes[j], companyName: companyName[k], fiscalYear: year[i] }).distinct('response').exec();
                        console.log("DDPPPPPPPPPPPCODE VALUESSSSSSSSSSS" , dpValues)

                        let std = await standardDeviation(dpValues);
                        console.log(".........", companyName[k] , year[i] , response)

                        if (response[0] === " ") {
                            let responseValue = { $set: { response: 'NA', performance: 'NA' } }
                            await clientData.updateOne({ DPCode: dpCodes[j], companyName: companyName[k], fiscalYear: year[i] }, responseValue).exec()
                        } else if (response[0] === 'NA') {                           
                            let responseValue = { $set: { performance: 'NA' } }
                            await clientData.updateOne({ DPCode: dpCodes[j], companyName: companyName[k], fiscalYear: year[i] }, responseValue).exec()
                        } else {
                            if (polarityCheck[0] === 'Positive') {
                                let value = await positive(dpCodes[j], companyName[k], year[i], dpValues, Number(response[0]), std)
                                await percentileCalc(value, dpCodes[j], companyName[k], year[i])
                                console.log(" CompanyName :::: " , companyName[k]  , dpCodes[j], " year ::: " , year[i] , " STDdeviation ::: " , std , " value  :: " , value)
                            } else if (polarityCheck[0] === 'Negative') {
                                let value = await negative(dpCodes[j], companyName[k], year[i], dpValues, Number(response[0]), std)
                                await percentileCalc(value, dpCodes[j], companyName[k], year[i]);
                                console.log(" CompanyName :::: " , companyName[k] ,dpCodes[j], " year ::: " , year[i] , " STDdeviation ::: " , std , " value  :: " , value)
                            }
                        }
...
async function missedDP(companyData, y, checkdp) {
    return new Promise(async (resolve, reject) => {
        for (let i = 0; i < checkdp.length; i++) {
            let clientDP = array[index];
        ...
        try {
            ...
            for (let i = 0; i < year.length; i++) {
                ...
                for (let j = 0; j < dpCodes.length; j++) {
                    ...
                    for (let k = 0; k < companyName.length; k++) {
                        const companyData = companyName[k];
